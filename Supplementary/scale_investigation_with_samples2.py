...
# File Name: scale_investigation_with_samples2.py
...
log_file_path = os.path.join(current_path, 'performance_log_increasing_sample_size.txt')
sample_sizes = [50, 100, 500, 1000, 5000, 10_000]
...
with io.open(log_file_path, 'w', buffering=1) as file:
    file.write("Sample_Size,Elapsed_Time_sec,Memory_Usage_MiB\n")
    for number_of_samples in sample_sizes:
        path_to_output = os.path.join(current_path, f'{number_of_samples}_samples')
        ...
        file.write(f"{number_of_samples},{elapsed_time},{max_memory}\n")
        file.flush()  # Ensure data is written to disk immediately
        print(f"Completed {number_of_samples} samples: Time {elapsed_time:.2f} sec, Memory {max_memory:.2f} MiB")
