...
X_orig, y = make_moons(n_samples=number_of_samples, noise=0.3, random_state=random_state)
...
print("ROC AUC of RFECV LR :", roc_auc_score(y_test, log_reg_prob, average='micro'))
print("Accuracy with full dataset (including noise) of the LR:", accuracy_score(y_test, full_pred))
print("ROC AUC with full dataset (including noise) of the LR:", roc_auc_score(y_test, full_prob, average='micro'))
...
plt.plot(fpr_log_rfecv, tpr_log_rfecv,
         label='RFECV with Logistic Regression (area = {:.2f})'.format(roc_auc_score(y_test, log_reg_prob,
                                                                                     average='micro')))
plt.plot(fpr_log_full, tpr_log_full,
         label='Logistic Regression (area = {:.2f})'.format(roc_auc_score(y_test, full_prob, average='micro')))


from sklearn.tree import DecisionTreeClassifier
# Setup RFECV with Decision Tree Classifier
decision_tree = DecisionTreeClassifier(random_state=random_state)
rfecv = RFECV(estimator=decision_tree)
rfecv.fit(X_train, y_train)
# Transform the test set based on the selected features
X_test_transformed = rfecv.transform(X_test)
...
decision_tree_full = DecisionTreeClassifier(random_state=random_state)
...
print("AUC of RFECV Decision Tree:", roc_auc_score(y_test, dt_prob, average='micro'))

print("Accuracy with full dataset (including noise) of the DT:", full_accuracy)
print("AUC with full dataset (including noise) of the DT:", roc_auc_score(y_test, full_prob, average='micro'))
...
plt.plot(fpr_dt_rfecv, tpr_dt_rfecv, label='RFECV with Decision Tree (area = {:.2f})'.format(roc_auc_score(y_test, dt_prob, average='micro')))
plt.plot(fpr_dt_full, tpr_dt_full, label='Decision Tree (area = {:.2f})'.format(roc_auc_score(y_test, full_prob, average='micro')))
...
xgboost_full = XGBClassifier(random_state=random_state)
...
print("AUC of RFECV XGBoost:", roc_auc_score(y_test, xgb_prob, average='micro'))

print("Accuracy with full dataset (including noise) of XGBoost:", full_accuracy)
print("AUC with full dataset (including noise) of XGBoost:", roc_auc_score(y_test, full_prob,average='micro'))
...
plt.plot(fpr_xgb_rfecv, tpr_xgb_rfecv,
         label='RFECV with XGBoost (area = {:.2f})'.format(roc_auc_score(y_test, xgb_prob, average='micro')))
plt.plot(fpr_xgb_full, tpr_xgb_full,
         label='XGBoost (area = {:.2f})'.format(roc_auc_score(y_test, full_prob, average='micro')))


# MOAgent performance
"""
MOA_roc = pd.read_csv('/media/dalco/FireCuda1/projects_21082024/MOAgent_revision/MOAgent_output/roc_auc_final_XGBclassifer_golden_features_test_roc_data.csv')
fpr = MOA_roc['FPR'].values  # Extracting the False Positive Rate
tpr = MOA_roc['TPR'].values  # Extracting the True Positive Rate
from sklearn.metrics import auc
# Compute the AUC
roc_auc = auc(fpr, tpr)
plt.plot(fpr,tpr, label='MOAgent (area = {:.2f})'.format(auc(fpr, tpr)))
"""

plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('micro-averaged ROC Curves of different models')
