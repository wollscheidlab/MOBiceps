"""
File: modifiedYellowbrick_ROCAUC.py
Author: Jens Settelmeier
The original function does not return classifier decision thresholds used to compute tpr and fpr for the ROC curve.
We extend the function, such that also the classifier thresholds are returned, which are computed from sklearn.
The returned threshold values correspond to the micro-averaged ROC curve, which we considere as more informative,
since class-imbalance is taken into account and the macro-average roc curve corresponding decision thresholds do not
exist by definition of macro-average. Averaging thresholds is not suitable to adjust decision threshold of a classifier.
Created: 03.01.2024
"""
...
class ROCAUC(ClassificationScoreVisualizer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # here we add the attribute "thresholds" which will capture the classifier decision thresholds which were used
        # from sklearn to compute the fpr and tpr of the ROC curve.
    ...
    def fit(self, X, y=None):
        ...
        print(
            "########### Note: Modified yellowbrick to consider Binary case as special case of Mulitclass case! #######"
        )
        ttype = (
            MULTICLASS  # type_of_target(y)     ### default is ttype = type_of_target(y)
        )
    ...
    def _score_micro_average(self, y, y_pred, classes, n_classes):
        ...
        # Compute micro-average
        self.fpr[MICRO], self.tpr[MICRO], thresholds = roc_curve(
            y.ravel(), y_pred.ravel()
        )
        self.thresholds[
            MICRO
        ] = thresholds  # Store the micro-average roc curve corresponding thresholds
